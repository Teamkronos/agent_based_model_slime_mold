...
to load-map
  ...
  ask patches [
    ...
    ifelse pcolor > 9.9 [
      ;; set node weight if pixel is colorful
      set node-weight pcolor - 9.9
    ] [
      ;; set elevation if pixel is grey
      set elevation (9.9 - pcolor)
    ]
  ]
...
to setup
  ...
  create-turtles population * count patches [
...
to go
  ask turtles [
    ;; turn toward patch with highest signal
    ;; or turn randomly if no unique maximum
    turn-toward-signal
    fd 1
  ...
  diffuse signal diffusion

  ask patches
  [
    set signal signal * evaporation
    set signal signal + node-weight
  ]

  tick
end

to turn-toward-signal
  let ahead patch-ahead sniff-distance
  let to-right patch-right-and-ahead 45 sniff-distance
  let to-left patch-left-and-ahead 45 sniff-distance

  let ahead-signal -1
  let to-right-signal -1
  let to-left-signal -1

  if (ahead != nobody) [
    set ahead-signal [signal] of ahead / ( 1 + abs ([elevation] of ahead - elevation))
  ]

  if (to-right != nobody) [
    set to-right-signal [signal] of to-right / ( 1 + abs ([elevation] of to-right - elevation))
  ]

  if (to-left != nobody) [
    set to-left-signal [signal] of to-left / ( 1 + abs ([elevation] of to-left - elevation))
  ]

  (ifelse to-left-signal > ahead-signal and to-left-signal > to-right-signal [
    lt 45
  ] ahead-signal > to-left-signal and ahead-signal > to-right-signal [
    ;; do not turn
  ] to-right-signal > to-left-signal and to-right-signal > ahead-signal [
    rt 45
  ] to-left-signal = ahead-signal and ahead-signal = to-right-signal [
    let r random 3
  ...
  ] to-left-signal = to-right-signal [
    let r random 2
...
map-file
map-file
"hokkaido_e_p.png" "hokkaido_e_np.png" "hokkaido_ne_p.png" "hokkaido_ne_np.png" "hawaii_e_p.png" "hawaii_e_np.png" "hawaii_ne_p.png" "hawaii_ne_np.png"
0
